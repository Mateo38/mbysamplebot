...
app.post('/webhook', function (req, res) {
    ...
    for (var i = 0; i < events.length; i++) {
        ...
        if (event.message && event.message.text) {
            ...
            processUserInput(event.message.text, function (recastAnswer) {
                console.log('Response from recast engine :' + recastAnswer);
                sendMessage(event.sender.id, recastAnswer);
            });
        }
    }
    res.sendStatus(200);
        ...
        if (response.intent() === 'find-race') {
            ...
            if (entityRaceType) {
                ...
                if (!raceType) {
                    ...
                    recAns('Tu as demand� une course de type non reconnu.');
                } else {
                    console.log('Tu as demand� une course de type ' + raceType + '.');
                    recAns('Tu as demand� une course de type ' + raceType + '.');
                }
            } else {
            ...
            if (entityRaceDate) {
                ...
                if (range) {
                    ...
                    recAns('Je n\'ai pas compris pour quelle p�riode tu recherches une course...');
                }
            } else {
                console.log('Je n\'ai pas compris s\'il y avait une p�riode pour la course recherch�e...');
                recAns('Je n\'ai pas compris s\'il y avait une p�riode pour la course recherch�e...');
            }
        } else {
